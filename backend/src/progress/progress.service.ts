...
export class ProgressService {
  ...
  async getCourseProgress(userId: number): Promise<CourseProgress[]> {
    ...
    for (const uc of enrolledCourses) {
      ...
      const completedInCourse = lessonIds.length > 0
        ? await this.progressRepository.find({
            where: { userId, lessonId: In(lessonIds), isCompleted: 1 },
          })
        : [];
      ...
      // Find last completed lesson or last lesson user was on
      ...
      if (completedInCourse.length > 0) {
        const latestCompleted = completedInCourse.sort(
          (a, b) => new Date(b.completedAt || 0).getTime() - new Date(a.completedAt || 0).getTime()
        )[0];
        lastStudyTime = latestCompleted.completedAt || null;
        if (!lastLesson) {
          const found = lessons.find(l => l.id === latestCompleted.lessonId);
          if (found) lastLesson = found;
        }
      }
