import { Controller, Get, Post, Put, Delete, Body, Param, Query, UseGuards, Req, NotFoundException, ForbiddenException } from '@nestjs/common';
...
export class ProjectsController {
  ...
  async update(@Param('id') id: string, @Req() req: any, @Body() data: Partial<{ name: string; type: string; content: string; projectData: string; isPublic: number }>) {
    const project = await this.projectsService.findOne(parseInt(id, 10));
    if (!project) throw new NotFoundException('Project not found');
    if (project.userId !== req.user.sub) throw new ForbiddenException('You can only update your own projects');
  ...
  async remove(@Param('id') id: string, @Req() req: any) {
    const project = await this.projectsService.findOne(parseInt(id, 10));
    if (!project) throw new NotFoundException('Project not found');
    if (project.userId !== req.user.sub) throw new ForbiddenException('You can only delete your own projects');
